...
#' @param dist          Population distribution. Use the \code{bindDist}
#'   function to specify a distribution. By default (\code{NULL} data is drawn
#'   from a normal distribution.)
#' @param model         Supply any custom model of class \code{SimSem}. When
#'   used, bypasses the loadings, population and latent.cor values.
#' @examples
...
#'
#' dist <- simsem::bindDist(skewness = 2, kurtosis = 7)
#' data <- getData(loadings = loadings, population = pop, latent.cor = 0.3, dist = dist)
#' @export
getData <- function(N = 100,
  ...
  # if model is null, assume CFA with loadings, population and latent.cor
  if (is.null(model)){
    ...
    # Factor correlation
    Psi <- matrix(NA, ncol(loading), ncol(loading))
    diag(Psi) <- 1
    RPS <- binds(Psi, latent.cor)

    # Error correlation
    error.cor <- matrix(0, nrow(loading), nrow(loading))
    diag(error.cor) <- 1
    RTE <- binds(error.cor)
  ...
  generate(model, n = N, indDist = dist)
}
